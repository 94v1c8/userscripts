...
// get login and supporter status
            if (!document.getElementById("user_information_header")){ // new system
                if(document.getElementsByClassName('anon_dropdown_container').length == 0){
                    loggedin=true;
                }
...
// not supporter or not logged in stream images
    ...
    function regetimg(){
        ...
        setTimeout(function(){ cimg.src = "https://cbjpeg.stream.highwebmedia.com/minifwap/"+fapbr+"?f="+ new Date().getTime();}, 50);//~6fps at 100ms load time
    }
...
//set interesting defchat settings in profile
    ...
    function getnotes(){
         ...
         var url="https://"+location.hostname+"/api/notes/for_user/"+roomname+"/";
